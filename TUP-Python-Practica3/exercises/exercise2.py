...
class Article:
    iva: float = 0.21 #variable que van a compartir todos los objetos de clase Article
    ...
    def calcular_precio(self) -> float:
        return round(self.costo + (self.costo + self.iva) - self.descuento,2) 
...
assert article.calcular_precio() == 2.21
# assertion error no da 1.21

article = Article("Auto", 1, 0.21)
assert article.nombre == "Auto"
assert article.calcular_precio() == 2
#assertion error no da 0.96
...
assert article.nombre == "Auto"
assert article.calcular_precio() == 2.21
#assertion error no daba 1.21

article = Article(costo=1, nombre="Auto", descuento=0.21)
assert article.nombre == "Auto"
assert article.calcular_precio() == 2
#assertion error no daba 0.96
...
article = Article(costo=1, nombre="Auto")
assert article.nombre == "Auto"
assert article.calcular_precio() == 2.25
#assertion error no daba con 1.25
# NO MODIFICAR - FIN